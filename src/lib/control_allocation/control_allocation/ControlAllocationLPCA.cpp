			  ...
			  LPCADiagnostics &diagnostics)
{
	...
	for (int i = 0; i < Rows; ++i) {
		...
		if (use_priority) {
			input_lower[i] = y_lower_par[i];
			input_higher[i] = y_higher_par[i];
		}
...
template<int Rows, int Cols, bool Supported = (Rows <= Cols)>
struct LPCAInvoker {
	static bool run(LPCAMethod,
			const float[ControlAllocation::NUM_AXES][ControlAllocation::NUM_ACTUATORS],
			const float[ControlAllocation::NUM_AXES],
			const float[ControlAllocation::NUM_AXES],
			const float[ControlAllocation::NUM_AXES],
			const float[ControlAllocation::NUM_ACTUATORS],
			const float[ControlAllocation::NUM_ACTUATORS],
			float[ControlAllocation::NUM_ACTUATORS],
			LPCADiagnostics &)
	{
		return false;
	}
...
template<int Rows, int Cols>
struct LPCAInvoker<Rows, Cols, true> {
	static bool run(LPCAMethod method,
			...
			LPCADiagnostics &diagnostics)
	{
		return runLPCA<Rows, Cols>(method, b_par, y_par, y_higher_par, y_lower_par,
					  actuator_min, actuator_max, output, diagnostics);
	}
};
			      ...
			      LPCADiagnostics &diagnostics)
{
	switch (cols) {
	case 4:
		return LPCAInvoker<Rows, 4>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);

	case 5:
		return LPCAInvoker<Rows, 5>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);

	case 6:
		return LPCAInvoker<Rows, 6>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);

	case 7:
		return LPCAInvoker<Rows, 7>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);

	case 8:
		return LPCAInvoker<Rows, 8>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);

	case 9:
		return LPCAInvoker<Rows, 9>::run(method, b_par, y_par, y_higher_par, y_lower_par,
					       actuator_min, actuator_max, output, diagnostics);
	}
...
ControlAllocationLPCA::allocate()
{
	...
	} else if (_method == Method::PCA && !hasPcaAxes()) {
		_diagnostics.solver_status = -1;
		_diagnostics.solver_err = kPCAUnavailableAxes;
		_used_fallback = true;
		allocated = allocateInv(actuator_delta);
...
ControlAllocationLPCA::allocateLPCA(ActuatorVector &actuator_delta)
{
	...
	const bool lpca_ok = dispatchLPCA(_num_active_rows, _num_actuators, lpca_method, _b_par, _y_par, _y_higher_par,
					  _y_lower_par, _actuator_delta_min, _actuator_delta_max, output,
					  lpca_diagnostics);
...
bool
ControlAllocationLPCA::hasPcaAxes() const
{
	return _num_active_rows == 3
	       && _active_rows[0] == ROLL
	       && _active_rows[1] == PITCH
	       && _active_rows[2] == YAW;
}
